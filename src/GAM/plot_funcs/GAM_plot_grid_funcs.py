...
def add_x_y_vals_context_plot(
    ...
    if smooths_plot:
        ...
        values_df.loc[values_df['linear'] == "nonlinear", ['x_surp', 'x_prev_surp']] += 50
...
def add_x_y_vals(
    ...
    if smooths_plot:
        ...
        values_df['y_prev_surp'] = values_df.apply(lambda row: y_base[row[y_col]] + y_increment, axis=1)  
...
def get_group_by_condition(df):
    if "critical_span_condition" in df.columns:
        ...
        return ['has_preview_condition', 'reread_condition', 'linear', 'context', 'context_type']
    else:
...
def _agg_valid_means(df: pd.DataFrame, group_by_cols: List[str], p_val_col: str, prev_p_val_col: str):
    return (df.groupby(group_by_cols)[
                [p_val_col, prev_p_val_col]]
            .apply(lambda x: x.apply(_valid_mean))
            .reset_index()
            )

def get_smooths_p_vals_symbols(df: pd.DataFrame, smooths_p_vals_path: str, correct_by_multiple_test: bool = False):
...
def save_corrected_smooths_p_vals_df(corrected_df: pd.DataFrame, smooths_p_vals_path: str):
...
def save_agg_symbols_df(smooths_p_vals_symbols: pd.DataFrame, smooths_p_val_df_path: str):
...
def _replace_text(
    text: str, 
    reread,
    col_names_df = pd.read_excel(src_path / Path("GAM/plot_funcs/GAM_plot_cols_names.xlsx"))
    ):
    col_names_df["fill_col"] = col_names_df["context column"].replace("filler", "fill")
    # 
    if "instFirstReadS" in text:
        condition = (
            (col_names_df["context column"]==text)
...
def draw_on_dll_img(dll_img_path, dll_p_vals):
    ...
    pil_image_lst = convert_from_path(dll_img_path) # This returns a list even for a 1 page pdf
    pil_image = pil_image_lst[0]
    # Initialize the drawing context with the image object
    draw = ImageDraw.Draw(pil_image)
    # Load a font
    font_path = 'DejaVuSans-Bold.ttf'  # Replace with the path to your downloaded font file
    ...
    for _, row in dll_p_vals.iterrows():
        ...
        symbol = row['symbol']
        color = 'black'
        draw.text(symbol_position, symbol, fill=color, font=ImageFont.truetype(font_path, symbol_size))
        ...
        if "context" in dll_p_vals.columns:
            ...
            if text == "NoName":
                continue
            color = 'black'
            draw.text(text_position, text, fill=color, font=ImageFont.truetype(font_path, text_font_size))
...
def draw_on_smooths_img(smooth_img_path, smooths_p_vals, context_reread_plot=False):
    # Open an existing image
    pil_image_lst = convert_from_path(smooth_img_path) # This returns a list even for a 1 page pdf
    pil_image = pil_image_lst[0]
    # Initialize the drawing context with the image object
    draw = ImageDraw.Draw(pil_image)
    color_dict = {
...
def add_p_vals_symbols(dll_img_path, dll_p_vals_path, smooth_img_path, smooths_p_vals_path, context_plot=False, fillers=False):
    # Read the CSV file
    ...
    dll_p_vals = add_x_y_vals(dll_p_vals, smooths_plot=False, context_plot=context_plot, fillers=fillers)
    ...
    smooths_p_vals = add_x_y_vals(smooths_p_vals, smooths_plot=True, context_plot=context_plot, fillers=fillers)
...
def add_p_vals_symbols_context_reread_plot(smooth_img_path, smooths_p_vals_path):
    ...
    logger.info("Smooths Plot | ----------------------")
    smooths_p_vals = get_smooths_p_vals_symbols(smooths_p_vals, smooths_p_vals_path)
    smooths_p_vals = smooths_p_vals[(smooths_p_vals["reread_condition"]==1) & (smooths_p_vals["context_type"]=="regime_context")].reset_index()
...
def create_grid(images, names, result_path, title=None, cols=None, rows=None):
    ...
    grid_image = Image.new('RGB', (cols * width, rows * (height + text_height) + title_height), (255, 255, 255))
    ...
    if title:
        title_x = (cols * width - len(title) * font_size) // 2  # Center the title horizontally
        title_y = 0
    ...
    for i, (img, name) in enumerate(zip(images, names)):
        ...
        if len(max(names, key=len)) < 20:
            text_x = x + 0.8 *( (width - font_size) // 2 ) # Center the text horizontally
        else:
...
def get_files(image_dir, model_name):
    ...
    if model_name != "all_models":
        linear_comp_files = [f for f in all_files if (f.endswith('_linear_comp_df.pdf') and (model_name in f))]
        surp_link_files = [f for f in all_files if (f.endswith('_surp_link.pdf') and (model_name in f))]
        dll_p_vals_files = [f for f in all_files if (f.endswith('_dll_p_vals_df.csv') and (model_name in f))]
        smooths_p_vals_files = [f for f in all_files if (f.endswith('_smooths_p_vals.csv') and (model_name in f))]
    else:
        linear_comp_files = [f for f in all_files if (f.endswith('_linear_comp_df.pdf'))]
        ...
        smooths_p_vals_files = [f for f in all_files if (f.endswith('_smooths_p_vals.csv'))]
...
def run_p_vals(model_name, analysis_type, results_dir, et_data_name, only_grid):
    ...
    linear_comp_files, surp_link_files, dll_p_vals_files, smooths_p_vals_files = get_files(image_dir, model_name)
    ...
    linear_comp_dict = {get_prefix(f, '_linear_comp_df.pdf'): os.path.join(image_dir, f) for f in linear_comp_files}
    surp_link_dict = {get_prefix(f, '_surp_link.pdf'): os.path.join(image_dir, f) for f in surp_link_files}
    dll_p_vals_dict = {get_prefix(f, '_dll_p_vals_df.csv'): os.path.join(image_dir, f) for f in dll_p_vals_files}
    smooths_p_vals_dict = {get_prefix(f, '_smooths_p_vals.csv'): os.path.join(image_dir, f) for f in smooths_p_vals_files}
    ...
    for prefix in linear_comp_dict:
        if prefix in dll_p_vals_dict:
            ...
            if not only_grid:
                add_p_vals_symbols(dll_img_path, dll_p_vals_path, smooth_img_path, smooths_p_vals_path)
...
def find_files_by_pattern(linear_comp_dict, dll_p_vals_dict, surp_link_dict, title, model_name):
    surp_link_images = []
    dll_images = []
    names = []
    for prefix in linear_comp_dict:
        if prefix in dll_p_vals_dict:
            ...
            if title == 'Different_Surprisal_Estimates': # Extract model names from the file names
                model_pattern = re.compile(r'FirstPassGD - (.+?)-Surprisal')
            elif title == 'GPT2 small | Different Contexts':
                model_pattern = re.compile(r'Context-(.+?)_re=FALSE') # Extract context type
            elif model_name == "gpt2":
                model_pattern = re.compile(r'(.+?) - gpt2-Surprisal') # Extract eye metric
            elif model_name == "EleutherAI-pythia-70m":
                model_pattern = re.compile(r'(.+?) - EleutherAI-pythia-70m-Surprisal') # Extract eye metric
            elif model_name == "EleutherAI-pythia-160m":
                model_pattern = re.compile(r'(.+?) - EleutherAI-pythia-160m-Surprisal') # Extract eye metric
            else:
    ...
    logger.info(f"Grid| n images: {len(surp_link_images)}, {len(dll_images)}, {len(names)}")
...
def _different_estimates_filter(names, surp_link_images, dll_images):
    ...
    filtered_surp_link_images = [img for name, img in zip(names, surp_link_images) if name != 'c']
...
def run_different_estimates_grid(names, surp_link_images, dll_images, title, with_p_dir):
    names, surp_link_images, dll_images = _different_estimates_filter(names, surp_link_images, dll_images)
...
def _sort_by_custom_order(title, model_name, names, surp_link_images, dll_images):
    ...
    if title == 'Different_Surprisal_Estimates': # Extract model names from the file names
        custom_order = pd.read_csv(PPL_PATH).sort_values(by=["sentence_level_ppl"])["model_id"].tolist()
    elif title == 'GPT2 small | Different Contexts':
        custom_order = ['p', 'p-p', 'fillerP-p', 'q-p', 'fillerQ-p', 'p-q-p', 'fillerP-fillerQ-p']
    elif model_name == "gpt2":
    ...
    filtered_data = [(name, surp_link, dll) for name, surp_link, dll in zip(names, surp_link_images, dll_images) if name in order_dict]
...
def run_grid(dll_p_vals_files, model_name, model_name_title, analysis_type, results_dir, et_data_name):
    ...
    # Create a dictionary to map prefixes to file paths
    linear_comp_dict = {get_prefix(f, '_linear_comp_df.pdf'): os.path.join(with_p_dir, f) for f in linear_comp_files}
    surp_link_dict = {get_prefix(f, '_surp_link.pdf'): os.path.join(with_p_dir, f) for f in surp_link_files}
    ...
    names, surp_link_images, dll_images = _sort_by_custom_order(title, model_name, names, surp_link_images, dll_images)
    logger.info(f"Grid| sorted names: {names}")
    logger.info(f"Grid| n images: {len(surp_link_images)}, {len(dll_images)}, {len(names)}")
    
    if title == 'Different_Surprisal_Estimates':
        run_different_estimates_grid(names, surp_link_images, dll_images, title, with_p_dir)
    else:
        create_grid(
            dll_images, 
            names, 
...
def add_anotations_context_plots(model_name, results_dir, et_data_name, article_level = "paragraph"):
    ...
    for fillers in ["TRUE", "FALSE"]:
        ...
        add_p_vals_symbols(context_dll_plot_path, context_dll_p_vals_path, context_GAM_plot_path, context_smooths_p_vals_path, context_plot=True, fillers=fillers)

def run_p_vals_and_grid(model_name, model_name_title, analysis_type, results_dir, et_data_name, only_grid = False):
    dll_p_vals_files = run_p_vals(model_name, analysis_type, results_dir, et_data_name, only_grid)
    run_grid(dll_p_vals_files, model_name, model_name_title, analysis_type, results_dir, et_data_name)
...
def test_add_ppl_to_names():
    ...
    names, surp_link_images, dll_images = _add_ppl_to_names(names, surp_link_images, dll_images)
    

if __name__ == "__main__":
    # surp_df = pd.read_csv("/src/GAM/cols_lists/surp_pythia70m_article_cols.csv")[["name", "reread"]]
    # for text, reread in zip(surp_df["name"], surp_df["reread"]):
    #     new_text = _replace_text(text, reread=reread)
